...
export function EmployeeRegistration() {
  ...
  const handleSubmit = async (event) => {
    ...
    const registrationResponse= await employeeRegistrationFetcher(formData, authToken);
    
    if(registrationResponse.data.statusCode === 200) {
      console.log(`Employee successfully added`);
    } else if(registrationResponse.data.statusCode >= 400 || registrationResponse.data.statusCode === 500) {
        ...
        <div className="col-12 col-lg-6 col-md-6">
          ...
          <div className="card px-2 py-3 py-lg-4 py-md-4 mt-5 justify-content-center form-card">
            ...
            <div className="card-body">
              <UserForm isEmployeeForm= {true} formData={formData} apiResponse={apiResponse} handleChange={handleChange} handleSubmit={handleSubmit}/>
            </div>
          </div>
        </div>
      </div>
